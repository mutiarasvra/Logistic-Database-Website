...
<html lang="en">
  ...
  <nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
    <div class="container">
      <a class="navbar-brand" href="../login/home.php"> EYA EXPRESS</a>
  ...
  <div class="block-31" style="position: relative;">
    <div class="owl-carousel loop-block-31 ">
      <div class="block-30 block-30-sm item" style="background-image: url('images/a1f7d3a41d2e4555d18e2fc9576df33f.jpg');" data-stellar-background-ratio="0.1">
        <div class="container">
...
<!-- 
								...
								<input type = "text" class = "form-control" name = "nama" value="test">
				...
				if (isset ($_POST ['save'])) {
					...
					$ambilkel = mysqli_query($koneksi, "SELECT * from penerima where kode_penerima='$kode4'") ;
					$isikel = mysqli_fetch_array ($ambilkel);
					$kel = $isikel ['kode_kel'];

					$ambilkec = mysqli_query($koneksi, "SELECT * from kelurahan where kode_kel='$kel'") ;
					$isikec = mysqli_fetch_array ($ambilkec);
					$kec = $isikec ['kode_kec'];

					$ambilkota = mysqli_query($koneksi, "SELECT * from kecamatan where kode_kec='$kec'") ;
					$isikota = mysqli_fetch_array ($ambilkota);
					$kota = $isikota ['kode_kabkota'];

					$ambilnkota = mysqli_query($koneksi, "SELECT * from kabkota where kode_kabkota='$kota'") ;
					$isinkota = mysqli_fetch_array ($ambilnkota);
					$nkota = $isinkota ['kabkota'];

					$ambila = mysqli_query($koneksi, "SELECT * from rute where kode_kabkota='$kota' AND kota_B = '$k2'") ;
					...
					$simpan = mysqli_query ($koneksi, "INSERT INTO resi (no_resi, tanggal_pengiriman, 
					tanggal_sampai, kode_penerima, kode_paket_barang, kode_jenis_packaging, kode_jenis_pengiriman, kode_jenis_asuransi,
					kode_jenis_tarif_cakupan, kode_jenis_dimensi, kode_status, berat)
					VALUES ('$noresi','$kode2','$kode3','$kode4','$kode5','$kode6',
					'$kode7','$kode8','$ya','$kode10','$kodes', '$kodeb')");
					?>
						...
						<?php  
							 ...
							 $koneksi= mysqli_connect( $db_host, $db_user, $db_pass, $db_name);
							  ...
							  $isitb = $isi2 ['tarif'] + $isi3 ['tarif'] + $isi4 ['tarif']  + $isi7 ['tarif'] + $isi5 ['tarif'] * $kodeb ;
							  $simpan3 = mysqli_query ($koneksi, "UPDATE resi set total_biaya='$isitb' where no_resi='$noresi'");} 
					...
					<table class="table table-hover">
                                  <thead>
									...
									<tr>
									  <td>Nama Pengirim                : </td>
									  <td><?php echo $isikel['nama'];?></td>		</tr>	
									 <tr> <td>Kota Asal                    : </td>
									  <td><?php echo $nkota;?></td>	</tr>		
