...
<div class="chit-chat-layer4">
	<div class="col-md-10 chit-chat-layer4-center">
                            ...
                            <div class="chit-chat-heading">
                                <br>  <H1><center> Tarif Jarak Cakupan </center> </H1><br>
                            </div>
                            <div class="table-responsive">
							 ...
							 $koneksi= mysqli_connect( $db_host, $db_user, $db_pass, $db_name); 
							  $ambil = mysqli_query ($koneksi, "SELECT * from jenis_tarif_cakupan") ;
							   $ambil1 = mysqli_query($koneksi, "SELECT * from jenis_cakupan,jenis_tarif_cakupan where jenis_cakupan.kode_jenis_cakupan=jenis_tarif_cakupan.kode_jenis_cakupan ") ;
							 ?> 
							  <?php
							  while ($isi = mysqli_fetch_array($ambil))
							  {
                                ...
                                <tr>
                                  <td><?php echo $isi ['kode_jenis_tarif_cakupan'];?></td>
                                  <td><?php echo $isi1 ['nama_jenis_cakupan'];?></td>
                                  <td><?php echo $isi ['rentang_jarak'];?></td> 
                                  <td><?php echo $isi ['tarif'];?></td> 
								</tr>
					  ...
					  <form method = "POST" enctype = "multipart/form-data">
							<div class = "form group">
								<label for = "kode_jenis_tarif_cakupan"> Kode </label>
								<input type = "text" class = "form-control" name = "kode_jenis_tarif_cakupan">
							</div>
							<br>
							<div class = "form group">
							<label for = "kode_jenis_cakupan"> Jenis Cakupan </label>
								<select name="kode_jenis_cakupan" id="kode_jenis_cakupan" class="form-control">
								<option value="">-- Pilih Jenis Cakupan -- </option>
								<?php 
								include 'config.php';
								$lo= mysqli_query ($koneksi, "SELECT * from jenis_cakupan") or die (mysqli_error($koneksi));
								$po= mysqli_fetch_array ($lo);
								 ?>
								 <?php do{ ?>
								 <option value="<?php echo $po['kode_jenis_cakupan']?>"><?php echo $po['nama_jenis_cakupan'] ?></option>
								 <?php }while ($po= mysqli_fetch_array ($lo));?>
								</select>
							</div>
							<br>
							<div class = "form group">
								<label for = "rentang_jarak"> Rentang Jarak </label>
								<input type = "text" class = "form-control" name = "rentang_jarak">
							</div>
							<div class = "form group">
							...
							<a href = "tarifjarakcak.php" class = "btn btn-warning"> Kembali </a>
					  </form>
				...
				if (isset ($_POST ['save'])) {
					$koneksi= mysqli_connect( $db_host, $db_user, $db_pass, $db_name); 
					$kode = $_POST ['kode_jenis_tarif_cakupan'];
					$kode1 = $_POST ['kode_jenis_cakupan'];
					$prov = $_POST ['rentang_jarak'];
					$trf = $_POST ['tarif'];
					$simpan = mysqli_query ($koneksi, "INSERT INTO jenis_tarif_cakupan VALUES ('$kode','$kode1', '$prov', '$trf')");
					echo "<br> <div class = 'alert alert-success text-center'> Data berhasil disimpan! </div>";
					echo "<meta http-equiv= 'refresh' content ='1;url=tarifjarakcak.php'>";
				}
				?>
