			...
			<div class="col-md-8 market-update-gd">
				<div class="market-update-block clr-block-1">
					<div class="col-md-8 market-update-left">
					              <br> <font face = "lucida sans" <b><H1><center> JUMLAH PENERIMA DARI KELURAHAN </font></center> </b></H1><br>
                            </div>
				...
				if (isset ($_POST ['check'])) {
					...
					<table class="table table-hover">
                                  <thead>
                                    <tr>
                                      <font color = "white"><b><th><h2>KODE</h2></th></b></font>
									  <font color = "white"><b><th><h2>NAMA </h2></th></b></font>
                                  </tr>
                              </thead>
					<?php
					$ambil3 = mysqli_query($koneksi, "SELECT * from penerima where kode_kel='$kja' ") ;
					
					$jum = mysqli_num_rows ($ambil3);
					
					$ambil4 = mysqli_query($koneksi, "SELECT * from kelurahan where kode_kel='$kja' ") ;
					$isi4 = mysqli_fetch_array ($ambil4);
					while($isi3 = mysqli_fetch_array ($ambil3)){
					$ambil2 = mysqli_query($koneksi, "SELECT * from kelurahan where kode_kel='$kja'") ;
					$isi1 = mysqli_fetch_array ($ambil2);
					$re = $isi1 ['kode_kel'];
					$na = $isi3 ['kode_penerima'];
					$ambil100 = mysqli_query($koneksi, "SELECT * from penerima,resi where '$na' = resi.kode_penerima ") ;
					$isi100 = mysqli_fetch_array ($ambil100);
					$ambil111 = mysqli_query($koneksi, "SELECT * from penerima where kode_penerima = '$na' ") ;
					$isi111 = mysqli_fetch_array ($ambil111);?>
					<br>
										<tbody>
					<tr>
                                <td><h2><?php echo $isi111 ['kode_penerima'];?>  </h2></td>
                                <td><h2><?php echo $isi111 ['nama'];?>   </h2></td>
						</tr>
					<?php	}	?>
					<?php$ambil3 = mysqli_query($koneksi, "SELECT * from penerima where kode_kel='$kja' ") ;
					
					$jum = mysqli_num_rows ($ambil3);
					
					$ambil4 = mysqli_query($koneksi, "SELECT * from kelurahan where kode_kel='$kja' ") ;
					$isi4 = mysqli_fetch_array ($ambil4);?>
					<h2><?php echo $isi4 ['kode_kel'];?><?php echo " - "?> <?php echo $isi4 ['kelurahan'];?></h2>
