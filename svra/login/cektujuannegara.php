			...
			<div class="col-md-8 market-update-gd">
				<div class="market-update-block clr-block-1">
					<div class="col-md-8 market-update-left">
					              <br> <font face = "lucida sans" <b><H1><center> JUMLAH PENERIMA DARI NEGARA </font></center> </b></H1><br>
                            </div>
							...
							<label for = "negara"> Negara </label>
								...
								<?php 
								include 'config.php';
								$lo= mysqli_query ($koneksi, "SELECT * from negara") or die (mysqli_error($koneksi));
								$po= mysqli_fetch_array ($lo);
								 ?>
								 <?php do{ ?>
								 <option value="<?php echo $po['kode_negara']?>"><?php echo $po['negara'] ?></option>
				...
				if (isset ($_POST ['check'])) {
					...
					$kn 	    = $_POST['kode_negara'];
					$ambil2 = mysqli_query($koneksi, "SELECT * from negara where kode_negara='$kn'") ;
					while ($isi1 = mysqli_fetch_array ($ambil2)){ 
					$re = $isi1 ['kode_negara'];
					$ambilo = mysqli_query($koneksi, "SELECT * from provinsi,negara where '$re'=provinsi.kode_negara ") ;
					$isio = mysqli_fetch_array ($ambilo);
					$ne = $isio ['kode_prov'];
					$ambil3 = mysqli_query($koneksi, "SELECT * from provinsi,kabkota where '$ne'=kabkota.kode_prov ") ;
					...
					<tbody>
					<tr>
                                <td><h2><?php echo $isi1 ['kode_negara'];?></h2></td>
                                <td><h2><?php echo $isi1 ['negara'];?></h2></td>
