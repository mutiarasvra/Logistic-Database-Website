			...
			<div class="col-md-8 market-update-gd">
				<div class="market-update-block clr-block-1">
					<div class="col-md-8 market-update-left">
					              <br> <font face = "lucida sans" <b><H1><center> JUMLAH PENERIMA DARI KECAMATAN </font></center> </b></H1><br>
                            </div>
                            <div class="table-responsive">
                            <form method = "POST" enctype = "multipart/form-data">
							<div class = "form group">
							<label for = "kecamatan"> Kecamatan </label>
								<select name="kode_kec" id="kode_kec" class="form-control">
								<option value="">-- Pilih Kecamatan -- </option>
								<?php 
								include 'config.php';
								$lo= mysqli_query ($koneksi, "SELECT * from kecamatan") or die (mysqli_error($koneksi));
								$po= mysqli_fetch_array ($lo);
								 ?>
								 <?php do{ ?>
								 <option value="<?php echo $po['kode_kec']?>"><?php echo $po['kecamatan'] ?></option>
				...
				if (isset ($_POST ['check'])) {
					...
					$ambil2 = mysqli_query($koneksi, "SELECT * from kecamatan where kode_kec='$kja'") ;
					?><?php
					$isi1 = mysqli_fetch_array ($ambil2);
					$re = $isi1 ['kode_kec'];
					$ambil3 = mysqli_query($koneksi, "SELECT * from kelurahan,kecamatan where '$re'=kelurahan.kode_kec ") ;
					$isi3 = mysqli_fetch_array ($ambil3);
					$ree = $isi3 ['kode_kel'];
					$ambil4 = mysqli_query($koneksi, "SELECT * from penerima where kode_kel ='$ree' ") ;
					while($isi4 = mysqli_fetch_array ($ambil4)){
					
					$jum = mysqli_num_rows ($ambil4);
					...
					<tbody>
					<tr>
                                <td><h2><?php echo $isi4 ['kode_penerima'];?></h2></td>
                                <td><h2><?php echo $isi4 ['nama'] ;?></h2></td>						
						</tr>
					<?php	}	?>
					
					</table>
					<h2><?php echo $isi1 ['kode_kec'];?> <?php echo " - "?><?php echo $isi1 ['kecamatan'];?></h2>
					<h2><?php echo "Jumlah : "?> <?php echo $jum;?></h2><?php?>
										<?php	}	?>
